  ...
  'zh-CN': {
    translation: {
      ...
      player: {
        ...
        play: '播放', pause: '暂停', stop: '停止', seek: '播放进度',
        openPlayback: '播放/监听',
      },
      common: {
        loading: '加载中…', cancel: '取消', retry: '重试', reset: '重置', refresh: '刷新', close: '关闭', confirm: '确定',
      ...
      training: {
        ...
        engineLogErrors: '{{count}} 条错误', engineLogEmpty: '暂无引擎输出。',
      ...
      envCheck: {
        title: '环境自检',
        idle: '尚未检测。', running: '正在检测运行环境…',
        passed: '✔ 环境检查通过，可以开始训练。',
        passedWithWarnings: '✔ 环境检查通过（{{count}} 项提示）。',
        failed: '✘ {{count}} 项检查未通过，训练已禁用。',
        errored: '环境检查未能完成。',
        blocked: '请先修复上方未通过的检查项，然后重新检测。',
        rerun: '重新检测',
        pythonVersion: 'Python {{version}}',
        installMissing: '安装缺失的依赖：',
        status: { ok: '通过', warn: '提示', fail: '未通过' },
        label: {
          python: 'Python 运行时', package: 'Python 依赖：{{name}}',
          device: '计算设备', memory: '系统内存', disk: '磁盘空间',
          writable: '数据目录', engine: 'Python 引擎',
        },
        fix: {
          package: '在命令行中运行上面的安装命令，然后点击“重新检测”。',
          python: '请安装 Python 3.9 或更高版本。',
          disk: '请清理磁盘空间后重新检测。',
          writable: '请检查目录权限，或确认杀毒软件是否拦截了本应用。',
          engine: '请重新安装应用；若使用 Windows，请将安装目录加入杀毒软件信任列表。',
          memory: '内存较小时建议使用“标准”模式训练。',
        },
      },
      cover: {
        ...
        // FC-02
        prereqSeparation: '完成音轨分离',
        datasetFilesTitle: '⓪ 分离数据',
        datasetReady: '数据就绪（{{files}}）',
        datasetNotReady: '请先完成步骤①的音轨分离，训练需要分离出的人声与伴奏。',
        // FC-03 / FC-04
        stepLockedTitle: '该步骤尚未解锁',
        stepLockedMessage: '请先完成步骤 {{first}}：{{steps}}',
        stepLockedTooltip: '请先完成：{{steps}}',
        nextTrainingData: '下一步：训练数据集 →',
        // FC-05
        preparingSource: '正在准备歌曲资源…',
        separatingPercent: '分离中… {{percent}}%',
        separationDone: '分离完成！',
        reSeparate: '重新分离',
        sepLogTitle: '分离日志（{{count}} 行）',
        sepStage: {
          loading: '正在载入音频与模型…',
          separating: '正在分离人声与伴奏…',
          splitting_harmony: '正在拆分主唱与和声…',
          dereverb: '正在去混响…',
          writing: '正在写入音轨文件…',
          done: '已完成',
        },
      ...
      library: {
        ...
        // FC-01
        downloadingResource: '正在下载歌曲资源…',
        downloadingPercent: '正在下载歌曲资源… {{percent}}%',
        localReady: '本地就绪',
      ...
      settings: {
        ...
        // Ticket T1：引擎启动超时可自定义，供杀毒软件扫描较慢或安装目录在网络盘上的机器使用。
        engine: {
          title: 'AI 引擎', description: '控制本地 Python 引擎的启动行为。',
          startupTimeout: '启动超时（秒）',
          startupTimeoutHint: '引擎首次输出前的等待时间，默认 {{seconds}} 秒。若首次超时，应用会自动重试一次；杀毒软件扫描较慢时可适当调大。',
        },
      },
    },
  },
  'en-US': {
    translation: {
      ...
      training: { title: 'Model Training', description: 'Fine-tune the AI singer\'s timbre using dry vocal recordings.', info: 'Model Info', name: 'Model name *', namePlaceholder: 'e.g. My Singer', epochs: 'Epochs', material: 'Training Material', noFiles: 'No files uploaded; synthetic demo data will be used.', mode: 'Training Mode', start: 'Start Local Training', training: 'Training…', complete: '✓ Training Complete', finalizing: 'Finalizing…', finalizingDesc: 'Training finished. Generating the demo clip and saving the model.', audition: 'Audition', trainAnother: 'Train Another Model', models: 'Your Models ({{count}})', demo: 'Demo', retrain: 'Retrain', delete: 'Delete', standard: 'Standard', professional: 'Professional', gpu: 'GPU', cpu: 'CPU', vram: 'VRAM', epoch: 'Epoch {{current}}/{{total}}', loss: 'Loss {{value}}', eta: 'ETA {{value}}', waiting: 'Waiting for engine…', logLabel: 'Training log', completeLog: 'Training complete!',
        ...
        // Ticket T2: device selection and CPU-fallback guidance.
        deviceMode: 'Compute device', currentDevice: 'Current device: {{device}}',
        gpuGeneric: 'Available', notDetected: 'Not detected',
        ...
        // Ticket T2: result summary. The pre-flight data-size question moved
        // into the self-check checklist (preflight.duration.*, Ticket P1).
      ...
      envCheck: {
        ...
        idle: 'Not checked yet.', running: 'Checking your environment…',
        passed: '✔ Environment check passed — you are ready to train.',
        passedWithWarnings: '✔ Environment check passed ({{count}} warning(s)).',
        failed: '✘ {{count}} check(s) failed — training is disabled.',
        errored: 'The environment check could not be completed.',
        blocked: 'Fix the failed checks above, then run the check again.',
        rerun: 'Run check again',
        pythonVersion: 'Python {{version}}',
        installMissing: 'Install the missing dependencies:',
        status: { ok: 'passed', warn: 'warning', fail: 'failed' },
        label: {
          python: 'Python runtime', package: 'Python package: {{name}}',
          device: 'Compute device', memory: 'System memory', disk: 'Disk space',
          writable: 'Data directory', engine: 'Python engine',
        },
        fix: {
          package: 'Run the command above in a terminal, then choose "Run check again".',
          python: 'Install Python 3.9 or newer.',
          disk: 'Free up disk space and run the check again.',
          writable: 'Check the folder permissions, or whether antivirus software is blocking the app.',
          engine: 'Reinstall the application; on Windows, add its install folder to your antivirus exclusions.',
      ...
      cover: { title: 'Cover Creation', description: 'Upload → Separate → Synthesize → Mix → Export', upload: 'Upload & Separate', song: 'Song file (WAV / FLAC / MP3)', chooseSong: 'Click to choose a song', separationMode: 'Separation mode', standard: 'Standard', enhanced: 'Enhanced', standardStems: '2 stems — vocals + accompaniment', enhancedStems: '3 stems — lead · harmony · accompaniment', startSeparation: 'Start Separation', separating: 'Separating…', stems: 'Stems — click Solo to preview', nextModel: 'Next: Select Model →', selectModel: 'Select AI Singer Model', noModels: 'No models trained yet. Go to Model Training first.', algorithm: 'Cover algorithm', v1: 'V1 — Fast', v2: 'V2 — High-Precision', v1Tagline: 'DTW + WSOLA · ≤10% real-time', v2Tagline: 'LSTM expression encoder · ≤50% RT', synthesize: 'Synthesize Cover', synthesizing: 'Synthesizing…', nextSynthesize: 'Next: Synthesize →', mix: 'Synthesize & Mix', mixer: 'Mixing Console', export: 'Next: Export →', exportTitle: 'Export Audio',
        ...
        // FC-02
        prereqSeparation: 'stem separation',
        datasetFilesTitle: '⓪ Separated data',
        datasetReady: 'Data ready ({{files}})',
        datasetNotReady: 'Run step ① separation first — training needs the separated vocals and accompaniment.',
        // FC-03 / FC-04
        stepLockedTitle: 'Step not unlocked yet',
        stepLockedMessage: 'Finish step {{first}} first: {{steps}}',
        stepLockedTooltip: 'Finish first: {{steps}}',
        nextTrainingData: 'Next: Training Dataset →',
        // FC-05
        preparingSource: 'Preparing song…',
        separatingPercent: 'Separating… {{percent}}%',
        separationDone: 'Separation complete!',
        reSeparate: 'Separate again',
        sepLogTitle: 'Separation log ({{count}} lines)',
        sepStage: {
          loading: 'Loading audio and models…',
          separating: 'Separating vocals and accompaniment…',
          splitting_harmony: 'Splitting lead and harmony…',
          dereverb: 'Removing reverb…',
          writing: 'Writing stem files…',
          done: 'Done',
        },
      ...
      library: {
        ...
        // FC-01
        downloadingResource: 'Downloading song…',
        downloadingPercent: 'Downloading song… {{percent}}%',
        localReady: 'Ready locally',
      ...
      settings: {
        ...
        // Ticket T1: adjustable for machines where a slow antivirus scan or a
        // network-mounted install directory delays the engine's first output.
        engine: {
          title: 'AI Engine', description: 'Controls how the local Python engine starts up.',
          startupTimeout: 'Start-up timeout (seconds)',
          startupTimeoutHint: 'How long to wait for the engine\'s first output. Default {{seconds}} s. A first timeout is retried once automatically; raise this if antivirus scanning makes launches slow.',
        },
      },
    },
  },
